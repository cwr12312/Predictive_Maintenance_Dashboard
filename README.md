...
4. **Live Prediction** — manual entry or file upload (CSV, JSON, HDF5, Pickle, MATLAB,
   LibSVM, ARFF), run any of the 6 models, confidence
...
No key is hard-coded anywhere in the project. This dashboard uses **Google Gemini**
exclusively as its LLM provider. To enable live mode, set the following:
...
Optionally override the model name with `DASHBOARD_LLM_MODEL` (defaults to
`gemini-3-flash-preview`). See `utils/llm_assistant.py` for the exact call site —
every LLM-dependent function there returns a `mode` of `"live"` or `"template"` so the
UI always tells you honestly which one produced the response.
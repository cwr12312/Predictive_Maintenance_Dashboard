...
DESIGN PRINCIPLE
-----------------
The six trained ML models (2D CNN, LSTM, Transformer, MAML, Meta-SGD, FBCL)
...
-------------------------------
No API key is hard-coded anywhere in this file. To enable real, live LLM
calls, set ONE of the following as an environment variable OR in
`.streamlit/secrets.toml` (see README.md for the exact steps):

    ANTHROPIC_API_KEY = "sk-ant-..."     # preferred provider
    OPENAI_API_KEY    = "sk-..."         # used only if the key above is absent
...
from config import (
    CLASS_DISPLAY_NAMES, FAULT_FAMILY, LLM_DEFAULT_ANTHROPIC_MODEL,
...
def get_active_provider() -> Optional[str]:
    """Returns 'anthropic', 'openai', or None if no key is configured."""
...
def _call_anthropic(system: str, user: str) -> str:
    import anthropic  # imported lazily so the package is only required if actually used
    client = anthropic.Anthropic(api_key=_get_secret("ANTHROPIC_API_KEY"))
    model = _get_secret("DASHBOARD_LLM_MODEL") or LLM_DEFAULT_ANTHROPIC_MODEL
...
def _call_openai(system: str, user: str) -> str:
    ...
    model = _get_secret("DASHBOARD_LLM_MODEL_OPENAI") or LLM_DEFAULT_OPENAI_MODEL
...
def call_llm(system: str, user: str) -> tuple[Optional[str], str]:
    """
    Attempts a real, live LLM call using whichever provider is configured.
    Returns (response_text, mode) where mode is:
      "live"     - a real API call succeeded, response_text is the model's real output
      "template" - no key configured, or the live call failed/errored
    response_text is None when mode == "template" (caller must build the fallback).
    """
    ...
    try:
        if provider == "anthropic":
            return _call_anthropic(system, user), "live"
        return _call_openai(system, user), "live"
    except Exception:
        # Never crash the dashboard on an API/network error — fall back silently.
        return None, "template"
